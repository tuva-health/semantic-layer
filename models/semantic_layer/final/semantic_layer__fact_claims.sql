...
from {{ ref('semantic_layer__fact_encounters') }} e
    left join {{ ref('semantic_layer__dim_encounter_provider') }} p
        on e.encounter_id = p.encounter_id
       and e.data_source = p.data_source
    left join {{ ref('ccsr', 'ccsr__dx_vertical_pivot') }} ccsr on e.primary_diagnosis_code = ccsr.code
...
select
  ...
  , mc.member_id
  , mc.member_month_id
  , {{ the_tuva_project.concat_custom(["mc.person_id", "'|'", "mc.data_source"]) }} as patient_source_key
  , {{ the_tuva_project.concat_custom(["mc.person_id", "'|'", the_tuva_project.yyyymm("mc.claim_start_date")]) }} as member_month_sk
  , {{ the_tuva_project.yyyymm("mc.claim_start_date") }} as year_month
  , sc.service_category_sk
  , mc.claim_id
  , mc.claim_line_number
  , mc.claim_type
  , mc.payer
  , mc.{{ the_tuva_project.quote_column('plan') }}
  , mc.claim_start_date
  , mc.claim_end_date
...
INNER JOIN encounter_sk esk
    ON mc.encounter_id = esk.encounter_id
   AND mc.data_source = esk.data_source
INNER JOIN {{ ref('semantic_layer__dim_data_source') }} as ds on mc.data_source = ds.data_source
WHERE mc.enrollment_flag = 1
